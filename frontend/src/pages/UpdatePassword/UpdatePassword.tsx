...
import { Lock, ArrowLeft, AlertCircle, CheckCircle2, Loader2 } from "lucide-react";
...
const UpdatePassword = () => {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      // Supabase automatically picks up the recovery token from the URL hash/query
      ...
      // Optional: Automatically redirect to login after 3 seconds
      setTimeout(() => {
        navigate("/login");
      }, 3000);
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <div className="flex flex-col gap-6 bg-black/40 border border-white/10 p-8 rounded-3xl backdrop-blur-md shadow-2xl">
        ...
        {success ? (
          ...
          <div className="flex flex-col items-center text-center gap-4 py-6">
            ...
            <Link
              to="/login"
              className="mt-4 text-xs text-[#d4af37] hover:underline flex items-center gap-2"
            >
          ...
          <form onSubmit={handleUpdatePassword} className="flex flex-col gap-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full border border-[#d4af37]/60 py-4 text-sm font-bold uppercase tracking-[0.25em] text-[#d4af37] bg-[#d4af37]/10 hover:bg-[#d4af37] hover:text-[#010102] transition-all duration-300 shadow-[0_0_25px_rgba(212,175,55,0.15)] flex items-center justify-center gap-3 rounded-xl cursor-pointer disabled:opacity-50"
            >
              {loading ? (
