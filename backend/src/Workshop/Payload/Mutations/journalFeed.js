...
export const fetchUserJournalFeed = async (journalOwnerUuid, loggedInUserUuid, fresh_load_pointer) => {
    ...
    const res = await pool.query(`SELECT id, uuid FROM profiles WHERE uuid = $1`, [journalOwnerUuid]);
    ...
    const queryParams = [journalOwnerNumericId];
    let queryText = `
    SELECT 
      c.*, 
