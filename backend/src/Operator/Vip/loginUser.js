...
export const loginUser = (req, res, next) => {
    passport.authenticate("local", (err, user, info) => {
        if (err)
            return next(err);
        if (!user)
            return next(new LoginError(info?.message || "user not found Register!!", 404));
        req.login(user, (err) => {
            if (err)
                return next(err);
            ...
            req.session.save(async (sessionErr) => {
                ...
                try {
                    ...
                    const { tasksFeed, next_post_timestamp } = await fetchGlobalTasksFeed(user.uuid, fresh_load_pointer);
                    const responseData = {
                        tasks: tasksFeed,
