...
import type { AuthenticatedRequest, UpdateAvatarResponseData } from "./types.js";
...
export const updateAvatar = async (
  ...
  res: Response<UpdateAvatarResponseData | { error: string }>,
  next: NextFunction
) => {
  ...
  try {
    ...
    const updatedProfile = await executeAvatarDatabaseTransactions(
      dbClient,
      user_uuid,
      avatar_url,
      uploadResult.bufferLength
    );
