...
export const getAlerts = async (req, res) => {
    try {
        ...
        const userId = req.user?.uuid || req.user?.id || req.user?.user_uuid;
        if (!userId)
...
export const markRead = async (req, res) => {
    try {
        ...
        let alertIdParam = req.params.alertId;
        if (Array.isArray(alertIdParam))
            alertIdParam = alertIdParam[0];
...
//# sourceMappingURL=getAlertsController.js.map